...
class SIDPAMISTGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G2 = torch.optim.Adam([{'params': self.netG2.netG.parameters()},
                                                ...
                                                lr=opt.lr, betas=(opt.beta1, 0.999), weight_decay=1e-5)
            self.optimizer_D = torch.optim.Adam([{'params': self.netG1.netD.parameters()}, 
                                                 {'params': self.netG2.netD.parameters()}],
                                                lr=opt.lr, betas=(opt.beta1, 0.999), weight_decay=1e-5)
    ...
    def forward(self):
        ...
        self.shadow_param_pred, self.alpha_pred, self.fake_free_shadow_image = self.netG2.forward_G(self.input_img, self.fake_shadow_image)
    ...
    def forward_D(self):
        ...
        self.pred_fake1, self.pred_real1 = self.netG1.forward_D(fake_AB.detach(), real_AB)
        ...
        self.pred_fake2, self.pred_real2 = self.netG2.forward_D(fake_AB.detach(), real_AB)
    ...
    def optimize_parameters(self):
        ...
        self.set_requires_grad([self.netG1.netD, self.netG2.netD], True)  # Enable backprop for D1, D2
        self.forward_D()
        self.optimizer_D.zero_grad()
        self.backward_D()
        self.optimizer_D.step()
     
        self.set_requires_grad([self.netG1.netD, self.netG2.netD], False) # Freeze D
        self.forward_D()
